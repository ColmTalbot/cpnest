...
def compute_weights(data, Nlive):
    ...
    start_data = np.concatenate(([-np.inf], data[:-Nlive]))
    end_data = data[-Nlive:]
    ...
    log_vols_end[0] = log_vols_start[-1] + np.log1p(-1 / Nlive)
    for i in range(len(end_data)-1):
        log_vols_end[i+1] = log_vols_end[i] + np.log1p(-1 / (Nlive - i))
...
def draw_posterior(data, log_wts):
    ...
    with associated log(weight) (of shape (Nsamples,)).
...
def draw_posterior_many(datas, Nlives, verbose=False):
    ...
    the corresponding Nlive number of live points. Will draw without
    repetition, and weight according to the evidence in each input run
    """
    # list of log_evidences, log_weights
    log_evs, log_wts = zip(*[compute_weights(data['logL'], Nlive)
                             for data, Nlive in zip(datas, Nlives)])
    if verbose:
        print('Computed log_evidences: {0!s}'.format((str(log_evs))))
    ...
    if verbose:
        print('Relative weights of input files: {0!s}'.format((str(fracs))))
    Ns = [fracs[i] / len(datas[i]) for i in range(len(fracs))]
    Ntot = max(Ns)
    fracs = [n / Ntot for n in Ns]
    if verbose:
        print('Relative weights of input files taking into account their '
              'length: {0!s}'.format((str(fracs))))

    posts = [draw_posterior(data, logwt)
             for (data, logwt) in zip(datas, log_wts)]
    if verbose:
        print('Number of input samples: {0!s}'.format(
            str([len(x) for x in log_wts])))
    if verbose:
        print('Expected number of samples from each input file {0!s}'.format(
    ...
    if verbose:
        print('Samples produced: {0:d}'.format(result.shape[0]))
    return result
...
def draw_N_posterior_many(datas, Nlives, Npost):
    """
    ...
    log_total_evidence = reduce(logaddexp, log_evs)
    Ns = [int(round(Npost * np.exp(log_ev - log_total_evidence)))
          for log_ev in log_evs]
    posts = [draw_N_posterior(data, logwt, N)
             for (data, logwt, N) in zip(datas, log_wts, Ns)]
...
def redraw_mcmc_chain(chain, verbose=False, burnin=True):
    ...
    """
    if verbose:
        print('Number of input samples: {0!s}'.format(chain.shape[0]))
    if burnin:
        chain = chain[chain.shape[0] / 2 - 1:]
    ACL = list()
    ...
    if verbose:
        print('Measured autocorrelation length {0!s}'.format(str(ACL)))
    chain = chain[::ACL]
    ...
    chain = chain[idx + 1]
    if verbose:
        print('Returned number of samples {0!s}'.format(str(chain.shape[0])))
