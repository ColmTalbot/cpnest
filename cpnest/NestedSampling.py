...
class _NSintegralState(object):
    ...
    def increment(self, logL, nlive=None):
        ...
        if logL <= self.logLs[-1]:
            print('WARNING: NS integrator received non-monotonic logL. {0:.5f} '
                  '-> {1:.5f}'.format(self.logLs[-1],logL))
        if nlive is None:
            nlive = self.nlive
        oldZ = self.logZ
        logt = -1.0/nlive
        ...
        if np.isfinite(oldZ) and np.isfinite(self.logZ) and np.isfinite(logL):
            self.info = np.exp(Wt - self.logZ)*logL + np.exp(
                oldZ - self.logZ) * (self.info + oldZ) - self.logZ
            if isnan(self.info):
                self.info = 0
    ...
    def finalise(self):
        ...
        """
        # Trapezoidal rule
        self.logZ = nest2pos.log_integrate_log_trap(
    ...
    def plot(self,filename):
        ...
        plt.title('{0} iterations. logZ={1:.2f} H={2:.2f} bits'.format(
            self.iteration, self.logZ, self.info*np.log2(np.e)))
        plt.grid(which='both')
...
class NestedSampler(object):
    ...
    def __init__(self, usermodel, sampler=None, nlive=1024, output=None,
                 verbose=1, seed=1, prior_sampling=False, stopping=0.1):
        """
        ...
        self.worst = 0
        self.logLmax = -np.inf
        self.logLmin = self.sampler.logLmin
        self.iteration = 0
        self.nested_samples = list()
        self.logZ = None
    ...
    def setup_output(self, output):
        ...
        chain_filename = "chain_{}_{}.txt".format(self.Nlive, self.seed)
    ...
    def consume_sample(self):
        ...
        _ = self.get_worst_n_live_points(1)
        logLtmp = -np.inf
        for k in self.worst:
            self.state.increment(self.params[k].logL)
            self.sampler.live = self.params[k]
            self.nested_samples.append(self.params[k])
            logLtmp = self.params[k].logL
        self.condition = - self.state.logZ + logaddexp(
            self.state.logZ, self.logLmax - self.iteration / self.Nlive)
        ...
        for k in self.worst:
            ...
            while True:
                loops += 1
                acceptance, sub_acceptance, self.jumps, proposed =\
                    self.sampler.produce_sample()
                if proposed.logL > self.logLmin:
                    # replace worst point with new one
                    self.params[k] = proposed
                    self.queue_counter = (self.queue_counter + 1) % 1
                    self.accepted += 1
                    break
                else:
            ...
            if self.verbose:
                sys.stderr.write(
                    "{0:d}: n:{1:4d} NS_acc:{2:.3f} S{3:d}_acc:{4:.3f} sub_"
                    "acc:{5:.3f} H: {6:.2f} logL {7:.5f} --> {8:.5f} dZ: "
                    "{9:.3f} logZ: {10:.3f} logLmax: {11:.2f}\n".format(
                        self.iteration, self.jumps*loops, self.acceptance, k,
                        acceptance, sub_acceptance, self.state.info,
                        logLtmp, self.params[k].logL, self.condition,
                        self.state.logZ, self.logLmax))
                sys.stderr.flush()
    ...
    def get_worst_n_live_points(self, n):
        ...
        self.logLmin = np.float128(self.params[n-1].logL)
    ...
    def reset(self):
        ...
        nthreads = 1
        with tqdm(
                total=self.Nlive, disable=not self.verbose,
                desc='CPNEST: populate samplers', position=nthreads) as pbar:
            while i < self.Nlive:
                self.sampler.live = self.model.new_point()
                # while i < self.Nlive:
                acceptance, sub_acceptance, self.jumps, self.params[i] =\
                    self.sampler.produce_sample()
                self.queue_counter = (self.queue_counter + 1) % 1
                if (self.params[i].logP != -np.inf) and\
                        (self.params[i].logL != -np.inf):
                    i += 1
                    pbar.update()
                    # break
        if self.verbose:
    ...
    def nested_sampling_loop(self):
        ...
        self.write_chain_to_file()
        self.write_evidence_to_file()
        print('Final evidence: {0:0.2f}\nInformation: {1:.2f}'.format(
            self.state.logZ, self.state.info))
